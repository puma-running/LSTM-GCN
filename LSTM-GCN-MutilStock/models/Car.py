...
class Car():
    """Prepare data, model training, model testing"""
    def __init__(self):
        self.emd = EMD()
    ...
    def loadtoloader(self):
        '''
        Data preparation: The data set is divided into two parts, one for training and one for testing
        Dic.point is the end point of the training and the start point of the test
        Dic.point_end Indicates the end point of the test
        '''
        arrs_train =[]
        arrs_test =[]
        codes = []
        ...
        for code in Dic.codes:
            ...
            if len(temp)==len(Dic.k_times) and (temp==Dic.k_times).all()==True:
                codes.append(code)
            else:
                Dic.log.info("A stock symbol with insufficient data length{}".format(code))
    ...
    def __getSamplesFromOnecode(self, arrs):
        ...
        for i in range(Dic.args.width1, len(arrs[0])-nwidth2):
            ...
            one_maxmin = []
            one_input = []
            # Process data on all stocks to form channels
            begin = i-nwidth1+1
            if begin >= 0:
