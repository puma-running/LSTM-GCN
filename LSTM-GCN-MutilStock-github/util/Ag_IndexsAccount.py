'''
代码随机从上交所或者深交所选klong只股票，
如果未下载过股票数据到本地，则从天勤上下载股票数据，
模型的参数在 paras中配置。
输出模型在这些股票上的回测年华收益率。
...
import numpy as np
import math
#import talib as tb  #pip install ta-lib
import time
import random
...
#(base) xiangdongdeMacBook-Pro:~ xiaohongmei$ xcode-select --install
...
class IndexsAccount():
    ...
    def __init__(self):
        # self.inrule = 1
        self.nminutes = 0
        self.nsuccess = 0   # 失败次数
        self.nfail =0       # 成功次数

        self.position_open = []     #开仓数量集合
        self.balance = []     #账户约的变动集合，初始余额1万

        #交易数据
        self.sTimes = []            #开仓时间、清仓时间集合
        self.lTimes = []            #开仓时间、清仓时间集合
        self.sIndexs = []           #开仓时、清仓时指数集合
        self.lIndexs = []           #开仓时、清仓时指数集合

        # self.moodindex = []           #交易时候的情绪指数集合

        #交易现金流情况
        # self.profits =[]              #清仓或止损每次的收益集合
        # self.fees = []                #手续费集合
        # self.margins = []             #股指保证金缴纳集合
        ...
        # 预测间隔（单位klines个数），在账户中用于止损和清仓时间判断，
        self.profitMinus = 0
        self.profitPlus = 0
    ...
    '''
    def __draw(self):
        x_data = self.time_open
        y_data = []
        for i in range(0,len(self.profits)):
            y_data.append(self.profits[i] - self.fees[i*2]-self.fees[i*2+1])
        
        plt.title(u'profit / open')
        plt.plot(x_data,y_data)
        plt.show()
    '''
    def setInitValue(self, hhBalance, hhPos):
        """ 从期货公司账户读取数据（权益，持仓）等，设置到IndexsAccount账户 """
        self.balance.append(hhBalance)
        self.pos = hhPos

    # def __draw_profits(self):
    #     x_data = np.arange(0,len(self.time_open))
    #     y_data = []
    #     for i in range(0,len(self.profits)):
    #         y_data.append((np.sum(self.profits[:i+1]) - np.sum(self.fees[:(i*2+1)+1])+self.balance[0])/self.balance[0])
